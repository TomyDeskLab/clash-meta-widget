> **此版本有已知故障，请改用 [preview.2（构建 10）](https://github.com/YL-SSSSu/clash-meta-widget/releases/tag/v1.0.0-preview.2)。** 构建 7 的旧卡片链接可能失效，点击时反复打开管理窗口。此前通过的是宿主 URL 入口测试，未覆盖桌面缓存卡片。本页旧 ZIP 没有替换，新版在独立发布页。
